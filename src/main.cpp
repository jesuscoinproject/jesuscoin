...
uint256 hashGenesisBlock( "0x98edebf3690d4d62a9c5f185ac22c4b218fa687e3f5de7d48a49bb61f183a81a" );
...
{
    int blocks_per_year = 365 * 24 * 60;
    int current_year = 1 + ( nHeight / blocks_per_year );
    int increment_divider = ( ( 1 + 33 ) * 33 ) / 2;

    int64 nSubsidy = ( ( MAX_MONEY / increment_divider ) * current_year ) / blocks_per_year;
    if( nHeight > ( blocks_per_year * 33 ) ) {
        nSubsidy = 0;
    }
...
static const int64 nTargetTimespan = 20 * 60; // Jesuscoin: 20 minutes
static const int64 nTargetSpacing = 60; // Jesuscoin: 1 minute
...
static const char* feeReceiverAddress="JJdvUd17Mx5htwQPAg2GmXayJf24miUcL5";

CScript *_fee_receiver = NULL;

CScript *getFeeReceiver() {
    if( _fee_receiver == NULL ) {
        CBitcoinAddress address = CBitcoinAddress( feeReceiverAddress );
        CScript scriptPubKey_ForFeeReceive;
        _fee_receiver = new CScript();
        _fee_receiver->SetDestination( address.Get() );
    }
    return _fee_receiver;
}
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    if( vtx[0].vout[1].scriptPubKey.GetID() != getFeeReceiver()->GetID() ) {
        printf( "CheckBlock: fee receiver address didnt match\n" );
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis block
        const char* pszTimestamp = "JAN. 22, 2014 2 Shooting Deaths a First in Ukraine Protests";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(2);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].SetEmpty();
        txNew.vout[1].nValue = 0 * COIN;
        txNew.vout[1].scriptPubKey = *getFeeReceiver();
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1390416103;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2091610244;
        ...
        assert(block.hashMerkleRoot == uint256("0xbccd09c923368d095d4583d5a5598bcd76557e81d98e3220bf656f298e70aa72"));
        block.print();
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    CBitcoinAddress address = CBitcoinAddress( feeReceiverAddress );
    CScript scriptPubKey_ForFeeReceive;
    scriptPubKey_ForFeeReceive.SetDestination( address.Get() );
    txNew.vout[1].scriptPubKey = scriptPubKey_ForFeeReceive;
